...
local CONFIG = {
    URLs = {
        ...
        chestRewards = "https://raw.githubusercontent.com/OllieIOW/hcr2eventdata/refs/heads/main/chestRewards.json",
        testJson = "https://raw.githubusercontent.com/OllieIOW/hcr2eventdata/refs/heads/main/testing/testJson.json"
    },
    patches = {
...
local function applyPatchesToEventData(eventData)
    ...
    if CONFIG.patches.patchChestRewards then
        chestRewardsData = downloadJsonFromURL(CONFIG.URLs.chestRewards)
        if chestRewardsData == nil then
            return nil, "Failed to download car rewards data"
        end
    end

    if CONFIG.patches.patchTestJson then
        testJsonData = downloadJsonFromURL(CONFIG.URLs.testJson)
        if testJsonData == nil then
            return nil, "Failed to download test JSON data"
        end
    end
    ...
    if chestRewardsData then
        table.insert(enabledRewardPatches, chestRewardsData)
    end
    if testJsonData then
        table.insert(enabledRewardPatches, testJsonData)
    end
...
local function patchEventFile(dumpOnly, specificEventFile)
    ...
    local modifiedData = eventData
    local replacementCount = 0
    
    -- Check if "(Ollies Patch)" already exists in name.value
    local alreadyPatched = eventData:match(
        '"name"%s*:%s*{%s*"value"%s*:%s*"[^"]*%(Ollies Patch%)"'
    )
    
    if not alreadyPatched then
        modifiedData, replacementCount = eventData:gsub(
...
local function getEnabledPatchesList()
    ...
    if CONFIG.patches.patchChestRewards then
        table.insert(enabledPatches, "Chest Shop Rewards")
    end
    if CONFIG.patches.patchTestJson then
        table.insert(enabledPatches, "Test JSON")
    end
...
local function showPreferencesMenu()
    local choice = gg.choice({
        ...
        "Chest shop rewards (" .. tostring(CONFIG.patches.patchChestRewards) .. ")",
        "Test JSON (" .. tostring(CONFIG.patches.patchTestJson) .. ")",
        "Back"
    ...
    elseif choice == 5 then
        CONFIG.patches.patchChestRewards = not CONFIG.patches.patchChestRewards
        gg.toast("Chest shop rewards is now: " .. tostring(CONFIG.patches.patchChestRewards))
        return false -- Stay in preferences menu
    elseif choice == 6 then
        CONFIG.patches.patchTestJson = not CONFIG.patches.patchTestJson
        gg.toast("Test JSON is now: " .. tostring(CONFIG.patches.patchTestJson))
        return false -- Stay in preferences menu
...
local function showMainMenu()
    ...
    -- Only add Dev Tools if not hidden
    if not CONFIG.hideDevTools then
        table.insert(menuItems, "Dev Tools")
    end
    table.insert(menuItems, "Exit")
    local choice = gg.choice(menuItems, nil, "Public Event Patcher")
    if choice == nil then
        return false
    elseif choice == 1 then
        local enabledPatches = getEnabledPatchesList()
    ...
    elseif choice == 3 then
        ...
        return false
    elseif choice == 4 then
        -- Check if this is "Dev Tools" or "Exit" based on CONFIG.hideDevTools
        if not CONFIG.hideDevTools then
            local returnToMain = false
            while not returnToMain do
    ...
    elseif choice == 5 then
        -- Only exists when Dev Tools is shown
        return true
    end
